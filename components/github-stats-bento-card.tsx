...
export function GitHubStatsBentoCard({ className }: { className?: string }) {
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    ...
    if (typeof window !== 'undefined' && window.innerWidth < 1024 && stats?.profileUrl) {
      e.preventDefault()
      e.stopPropagation()
      window.open(stats.profileUrl, '_blank', 'noopener,noreferrer')
    }
  }
  ...
  return (
    <div
      className={cn(
        ...
        stats?.profileUrl && "cursor-pointer lg:cursor-default",
        className
      )}
    ...
    >
      ...
      <div className="p-4 relative z-10">
        <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-1 transition-all duration-300 lg:group-hover:-translate-y-10">
          <Github className="h-12 w-12 origin-left transform-gpu text-neutral-400 transition-all duration-300 ease-in-out group-hover:scale-75" />
